...
class ConditionalConv2d(nn.Module):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.s = nn.Linear(9, args[1])
        self.b = nn.Linear(9, args[1])
...
if __name__ == '__main__':
    class net(nn.Module):
        def __init__(self):
            ...
            self.conv = nn.Conv2d(1, 2, 3, padding=1, stride=1)
            self.a = nn.Linear(5, 2)
            nn.init.constant_(self.a.bias.data, 0.)
            nn.init.constant_(self.a.weight.data, 1.)

        def forward(self, x, lmbda):
            x = self.conv(x)
            print('x', x)
            weight = self.a(lmbda).unsqueeze(-1).unsqueeze(-1).unsqueeze(0)
            print('weight', weight)
            return x * weight


    net_ = net()
    image = torch.rand_like(torch.zeros(3, 1, 3, 3))
    print('image', image)
    # print(image)
    lmbda = torch.tensor([1., 0., 0., 0., 0.])
    out = net_(image, lmbda)
    print(out)
